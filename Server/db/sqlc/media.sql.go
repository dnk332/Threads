...
VALUES ($1, 'image', $2, 'post', $3) RETURNING id, link, type, order_column, created_at, reference_object, reference_object_id
`
...
func (q *Queries) AddPostImage(ctx context.Context, arg AddPostImageParams) (Media, error) {
	row := q.db.QueryRow(ctx, addPostImage, arg.Link, arg.OrderColumn, arg.ReferenceObjectID)
	var i Media
	err := row.Scan(
		&i.ID,
		&i.Link,
...
// Get image
func (q *Queries) GetImagesForPost(ctx context.Context, referenceObjectID int64) ([]Media, error) {
	rows, err := q.db.Query(ctx, getImagesForPost, referenceObjectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
